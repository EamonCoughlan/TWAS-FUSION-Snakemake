...
M = length(models)
sample<-gsub("data/transcript_beds_with_pheno/", "", opt$bfile)
opt$tmp<-paste(opt$tmp,sample,sep="")
...
weights.lasso = function( input , hsq , snp , out=NA ) {
	...
	cat( paste(out,".lasso",sep='') , "  did not exist\n" )
	eff.wgt = rep(NA,length(snp))
	} else {
...
#opt$grm<-paste(opt$tmp,"opt$bfile", "")
# 1. generate GRM
arg = paste( opt$PATH_plink," --allow-no-sex --bfile ",opt$bfile," --make-grm-bin --out ",opt$tmp,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
