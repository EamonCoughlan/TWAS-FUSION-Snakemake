...
if ( opt$plot ) {
	...
	if ( !file.exists("glist-hg38") ) {
		if ( file.exists( paste(script.dir,"/glist-hg38",sep='') ) ){
		glist = read.table(paste(script.dir,"/glist-hg38",sep=''),as.is=T)
		glist = glist[glist[,1] == chr,]
		} else {
		cat( "WARNING: glist-hg38 file listing gene locations (with header: CHR P0 P1 ID) needed for locus plots\n" , file=stderr() )
		glist = matrix(nrow=0,ncol=4)
		}
	} else {
		glist = read.table("glist-hg38",as.is=T)
		glist = glist[glist[,1] == chr,]
	}
