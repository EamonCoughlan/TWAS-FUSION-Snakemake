...
profile<-matrix(0,length(models), 13)
colnames(profile)<-c("id", "nsnps", "hsq", "hsq.se", "hsq.pv", "top1.r2", "blup.r2", "enet.r2", "lasso.r2", "top1.pv", "blup.pv", "enet.pv", "lasso.pv")
...
for(i in models){
	...
	tryCatch({
		...
		profile[i, "blup.r2"]<-cv.performance["rsq", "blup"]
		...
		profile[i, "blup.pv"]<-cv.performance["pval", "blup"]
